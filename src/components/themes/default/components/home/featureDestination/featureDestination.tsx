        ...
        <div className="w-full max-w-[1200px] mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center mt-6">
            {[1, 2, 3, 4, 5, 6].map((item) => (
              <div key={item} className="animate-pulse bg-gray-300 rounded-[65px] w-full max-w-[415px] h-[577px]"></div>
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-[1200px] mx-auto">
          ...
          {destinations.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">No featured destinations available at the moment.</p>
            </div>
          ) : (
              ...
              {destinations.map((destination) => (
                ...
                >
                  <div className="relative overflow-hidden rounded-[55px] m-3
                                ...
                                md:h-[260px]
                                lg:h-[393px]">
                    <img src={destination.image} alt={`${destination.name}`} className="w-full h-full object-cover" onError={(e) => {         // Simple fallback agar image load nahi hoti
                        (e.target as HTMLImageElement).src = "./images/destinations/default.jpg";
                      }}
                    />
                  </div>
                  <div className="p-3">
                    <h3 className="text-xl font-extrabold text-gray-900 mb-4 pl-4
                                 sm:text-2xl sm:mb-5 md:text-xl md:mb-4 lg:text-2xl lg:mb-6"
                        style={{ fontFamily: 'Urbanist, sans-serif' }}>
                      {destination.location}, {destination.country}
                    </h3>
                    <div className="flex items-center gap-3 sm:gap-4">
                      <button onClick={() => handleExploreNow(destination)}
                        className="flex-1 mx-1 ml-3 bg-[#163C8C] hover:bg-blue-700 text-white font-medium
                                 py-3 px-3 text-sm sm:py-3 sm:px-3 sm:text-base md:py-3 md:px-3 md:text-sm lg:py-4 lg:px-3 lg:text-base
                      ...
                      <button
                        ...
                        aria-label={`${likedDestinations.has(destination.id) ? 'Unlike' : 'Like'} ${destination.location}, ${destination.country}`}
                      >
                        <svg
